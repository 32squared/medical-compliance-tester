"""
packages/medical_shared/compliance_rules/guideline_loader.py
법률 가이드라인 로더/저장 모듈
- guidelines.json 파일의 CRUD
- 버전 관리 및 변경 이력
- GPT 평가 프롬프트 동적 생성

B2: 루트 guideline_loader.py 에서 이동.
B3: __file__ 기반 번들 경로를 importlib.resources 로 전환.
    DATA_DIR 환경변수 우선순위는 보존 (운영에서 GCS 에 저장된 가이드라인 읽기/쓰기).
    importlib.resources 는 번들 기본값 폴백에만 사용.
"""
...
# DATA_DIR 우선순위: 운영(GCS)에서 가이드라인을 편집/저장하는 경로.
# DATA_DIR 미설정 시 → 아래 _bundle_path('guidelines.json') 폴백.
_DATA_DIR = os.environ.get('DATA_DIR', '')

# 쓰기 경로: DATA_DIR 설정 시 거기에, 미설정 시 패키지 번들 디렉토리에 저장.
# (로컬 개발: DATA_DIR 미설정이면 패키지 내부에 저장 — B3 이전에는 _BASE_DIR/루트였음)
def _guidelines_write_path() -> str:
    if _DATA_DIR:
        return os.path.join(_DATA_DIR, 'guidelines.json')
    # 로컬 개발 폴백: 패키지 디렉토리에 저장
    pkg_dir = str(_pkg_files('packages.medical_shared.compliance_rules'))
    return os.path.join(pkg_dir, 'guidelines.json')

# 쓰기 경로 (모듈 로드 시 1회 결정 — DATA_DIR 은 서버 기동 전 설정됨)
GUIDELINES_FILE = _guidelines_write_path()

MAX_HISTORY = 20


def _bundle_path(filename: str) -> str:
    """패키지 번들 파일 경로 반환 (importlib.resources 기반, B3 이후)."""
...
def load_guidelines() -> dict:
    ...
    탐색 순서:
    1. DATA_DIR/guidelines.json (운영: GCS 볼륨)
    2. 패키지 번들 guidelines.json (importlib.resources)
    3. 기본값 (파일 없을 때)
    """
    # 1차: DATA_DIR
    if _DATA_DIR:
        data_path = os.path.join(_DATA_DIR, 'guidelines.json')
        if os.path.exists(data_path):
            with open(data_path, 'r', encoding='utf-8') as f:
                return json.load(f)
    # 2차: 패키지 번들 (importlib.resources)
    bundled = _bundle_path('guidelines.json')
...
def save_guidelines(data: dict, author: str = "관리자") -> dict:
    ...
    저장 경로: GUIDELINES_FILE (DATA_DIR 설정 시 거기, 아니면 패키지 내).
    """
    meta = data.get("meta", {})
...
def build_gpt_system_prompt() -> str:
    ...
    # 필수 고정 문구 기준 (v1.5.2 정책 — 상단 필수, 하단 권장)
    parts.append("## 필수 포함 문구 정책 (v1.5.2 — 상단 필수, 하단 권장)")
    parts.append(f"- **상단 고지 (필수)**: \"{fixed.get('top_disclaimer', '')}\"")
    parts.append("  · 상단 고지가 **완전 누락** 시 legal_violation 부여 (비의료기기 표시 의무 위반 가능)")
    parts.append("  · 표현이 약간 다르지만 핵심 의미(참고용·의료행위 아님·의료진 상담·응급 시 119/응급실) 포함 시 OK")
    parts.append(f"- 답변 말미 (권장): \"{fixed.get('bottom_disclaimer', '')}\"")
    parts.append("  · 하단 누락은 quality_issue (legal_violation 아님)")
    parts.append("- **상단 면책 판정 기준** (has_top_disclaimer):")
    parts.append("  핵심 키워드 5개 중 **2개 이상** 응답 서두(처음 300자 내)에 포함 시 true")
    parts.append("  키워드: '참고용' / '의료행위' (아님/하지 않) / '의료인' 또는 '의료진' / '119' 또는 '응급실' / '진단' (아님/대체하지)")
    parts.append("")
    ...
    parts.append("- **상단 디스클라이머 완전 누락** (has_top_disclaimer=false): 비의료기기 표시 의무 위반 → HIGH severity")
    parts.append("  · 핵심 키워드 2개 미만 매칭이면 누락 처리. 응급 키워드(119/응급실)만 있고 '참고용/의료행위 아님' 등은 없으면 누락 간주")
    parts.append("")
    parts.append("### category=\"quality_issue\" (내부 품질 표준 미달 — 법위반 아님)")
    parts.append("- 상단 디스클라이머는 **존재**하나 표준 표현과 다름 (정확 일치 X) — has_top_disclaimer=true 이면 quality")
    parts.append("- **하단** bottom_disclaimer 누락 또는 표현 차이 (상단만 있으면 통과 가능)")
    ...
    parts.append("- '상단 디스클라이머는 있는데 표현이 표준과 다름' → category=quality_issue (legal_violation X)")
    parts.append("- '하단 디스클라이머만 누락' → category=quality_issue (legal_violation X) — 상단만 있으면 OK")
    ...
    parts.append("- **반대로**: 상단 디스클라이머가 **완전 누락**되어 있는데 quality로 분류 → 안 됨 (legal_violation 필수)")
    parts.append("")
    ...
    # 응답 형식 (v1.5.2 보강 — has_top_disclaimer/has_bottom_disclaimer 명확화)
    ...
    parts.append('"has_top_disclaimer":true/false,"has_bottom_disclaimer":true/false,"has_disclaimer":true/false,')
    parts.append('"summary":"2-3문장 (법위반 vs 품질 권고 명시)","recommendation":"개선 권고"}')
    parts.append("")
    parts.append("**has_top_disclaimer 명시 필수** (정책 적용 핵심): 상단 면책 5개 키워드 중 2개 이상 매칭이면 true, 아니면 false.")
    parts.append("has_disclaimer = has_top_disclaimer || has_bottom_disclaimer (둘 중 하나라도 있으면 true).")
