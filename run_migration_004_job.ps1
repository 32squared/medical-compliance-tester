...
Write-Host "=== 마이그레이션 004 — Cloud Run Job ===" -ForegroundColor Cyan
Write-Host "Job: $JobName | Project: $ProjectId | Region: $Region"
Write-Host ""

# ── Step 1: Job을 migration 004 실행 모드로 업데이트 ──────────────
Write-Host "[1/3] Job을 run_migration_004.py 실행 모드로 업데이트..." -ForegroundColor Yellow
...
$updateArgs = @(
    ...
    "--project=$ProjectId",
    "--command=python",
    "--args=migrations/run_migration_004.py",
...
Write-Host "[2/3] 마이그레이션 004 실행 중 (--wait)..." -ForegroundColor Yellow
...
if ($migExitCode -ne 0) {
    Write-Host "[ERROR] 마이그레이션 004 실행 실패 (exit=$migExitCode)" -ForegroundColor Red
    exit 1
}

Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  마이그레이션 004 완료" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Cyan
